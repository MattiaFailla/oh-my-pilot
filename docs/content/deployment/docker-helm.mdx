...
<Callout type="info">
The official image is available at `ghcr.io/qf-studio/oh-my-pilot`. It bundles the oh-my-pilot binary, pinned OMP runtime, Git, and the GitHub CLI (`gh`) in a single Ubuntu-based image running as a non-root user.
</Callout>
...
mkdir -p omp-profile # Populate through your OMP profile-management workflow
```
